  ...
  var directiveName = 'socialshare'
    ...
    , socialshareConfigurationProvider = function socialshareConfigurationProvider() {
        ...
        return {
            ...
            'configure': function configure(configuration) {
                ...
                for (; configIndex < socialshareConfigurationDefault.length; configIndex += 1) {
                    var configElement = socialshareConfigurationDefault[configIndex];

                    if (configElement && configElement.provider) {
                        for (; internIndex < configuration.length; internIndex += 1) {
                            if (configuration[internIndex] && configuration[internIndex].provider == configElement.provider) {
                                ...
                                var configurationKeys = Object.keys(configElement.conf)
                                    ...
                                    for (; configurationIndex < configurationKeys.length; configurationIndex += 1) {
                                        ...
                                        if (aConfigurationKey && configuration.conf[aConfigurationKey]) {

                                            configElement.conf[aConfigurationKey] = configuration.conf[aConfigurationKey];
                                        }
                                    }
    ...
    , socialshareDirective = /*@ngInject*/ function socialshareDirective($window, $location, socialshareConf) {
        ...
        var linkingFunction = function linkingFunction($scope, element, attrs) {
            ...
            /*
            for (key in configurationElement.conf) {
                if (configurationElement.conf.hasOwnProperty(key)) {
                    var attributeName = 'socialshare' + key.substring(0, 1).toUpperCase() + key.substring(1);
                    (function observePropValues(keyName) {
                        attrs.$observe(configurationElement.provider, function observeAttrs(value) {
                            if (value) {
                                configurationElement.conf[keyName] = value;
